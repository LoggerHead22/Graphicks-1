...
void MainWindow::paintEvent(QPaintEvent *event) {
    ...
    auto fnLines = functionLines(currentFunction(), view);
    auto apprxLines = functionLines(std::bind(&MainWindow::f_aprox_polin, this, std::placeholders::_1), view);
    auto fnMinusApprxLines = functionLines([&](double x) { return std::abs(currentFunction()(x) - f_aprox_polin(x)); }, view);
    ...
    QPair<double, double> fnMinusApprxMinMax = functionMinMax([&](double x) { return std::abs(currentFunction()(x) - f_aprox_polin(x)); }, view);
    QPair<double, double> minMax = {qMin(fnMinMax.first, apprxMinMax.first),
                                    qMax(fnMinMax.second, apprxMinMax.second)};
    if (!drawFunction && drawApproximated) {
        minMax = fnMinusApprxMinMax;
    }

//    qDebug() << std::abs(minMax.first - minMax.second) << std::numeric_limits<double>::epsilon();
    if (std::abs(minMax.first - minMax.second) <= std::numeric_limits<double>::epsilon()) {
        minMax.first = minMax.first - 1;
        minMax.second = minMax.second + 1;
    }
         ...
         i += plotRect.width() / numCount, x += (view.second - view.first) / numCount)
    {
        ...
        painter.drawText(i, plotRect.bottom() + painter.fontMetrics().height(), QString::number(x));
    ...
    {
        painter.setPen(numPen);
        painter.drawText(plotRect.left(), i, QString::number(y));
        painter.setPen(gridPen);
