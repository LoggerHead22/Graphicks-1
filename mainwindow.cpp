...
struct QPainterD {
    ...
    QPainterD(QPainter& painter)
        : painter(painter)
    {}

    QPoint transformed(const QPointF& p) {
        QPoint ret;
        ret.rx() = ((double(p.x()) + t1x) * s1x) + t2x;
        ret.ry() = ((double(p.y()) + t1y) * s1y) + t2y;
        return ret;
    }
    ...
    void drawRect(const QRectF& rect) {
        painter.drawRect(QRectF(transformed(rect.topLeft()), transformed(rect.bottomRight())));
    }

    void drawText(int x, int y, const QString& text) {
        QPoint t = transformed(QPointF(x, y));
        painter.drawText(t.x(), t.y(), text);
    }

    void drawLine(int x1, int y1, int x2, int y2) {
        QPoint a = transformed(QPointF(x1, y1));
        QPoint b = transformed(QPointF(x2, y2));
        painter.drawLine(a, b);
    }

    void drawLine(const QLineF& line) {
        QPoint a = transformed(line.p1());
        QPoint b = transformed(line.p2());
        painter.drawLine(a, b);
    }

    void drawLines(QVector<QLineF>& lines) {
        for (auto& it : lines) {
            it = QLineF(transformed(it.p1()), transformed(it.p2()));
        }
        painter.drawLines(lines);
    }

    void fillRect(const QRectF& rect, const QColor& color) {
        painter.fillRect(QRectF(transformed(rect.topLeft()), transformed(rect.bottomRight())), color);
    }
...
//    if (kit == 2) painter.drawText(1200, 40, "Набор = погрешность newton");
    ...
    painter.t1x = -viewRect.center().x();
    painter.t1y = -viewRect.center().y();
    painter.s1x = plotRect.width() / (view.second - view.first);
    painter.s1y = plotRect.height() / -(minMax.second - minMax.first);
    painter.t2x = plotRect.center().x();
    painter.t2y = plotRect.center().y();
...
//    painter.translate(-viewRect.center());
    ...
    if (isZooming) {
        ...
        painter.fillRect(QRectF(QPointF(zoomStart, minMax.first), QPointF(zoomEnd, minMax.second)), QColor(255, 128, 8, 128));
    }
    //painter.restore ();
